    ...
    while i <= len(str(num)) - 1:
        ...
        if curr >= prev:
            if curr == prev:
                ...
                if i > 1 and curr == int_pos(num, i-2):
                    # removing two because in case of a triple block this, and previous do not count
                    adjacent_duplicate -= 2
                    if i > 2 and curr == int_pos(num, i-3):
                        adjacent_duplicate += 1
            i += 1
        else:
            return 0
